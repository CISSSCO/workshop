...
** Parallel Execution
Making progression on more than one task at the exact same time.
...
#+begin_src bash :session gcc14 :results output
#gcc --version
gcc hello.c -o hello.out -fopenmp
#+end_src
...
#+begin_src bash :tangle run.sh :session gcc14 :results output
#!/bin/bash
#SBATCH --job-name=openmp        ### Job Name
#SBATCH --time=00:03:00              ### WallTime
#SBATCH --nodes=1                    ### Number of Nodes
#SBATCH --ntasks-per-node=1          ### Number of tasks (MPI processes)
#SBATCH -n=$OMP_NUM_THREADS          ### Number of cores (OMP threads)

./$1
#+end_src

* Using run.sh script
- Create a file called run.sh in your folder and paste the content inside the file.
...
- Now you can run your code using below command:

#+begin_src bash :session gcc14 :results output
./run.sh ./a.out
#+end_src

- You can also use this if previous is not working:
...
Example: Number of threads = 4
(Threads execute concurrently within parallel region.)
...
* Assignment
Create a parallel matrix addition program.
